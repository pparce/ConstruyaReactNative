...
export default class EditDireccion extends Component {
    ...
    _saveInformation = () => {
        let url = ApiService.instance.buildUrlById(ApiService.EDIT_ADDRESS, this.state.data.id);
        ApiService.instance.put(url, this._buildJSON()).then(
            response => {
                if (response.id) {
        ...
        return (
            <View style={{ flex: 1 }}>
                <Appbar.Header style={[Theme.style.toolbar]}>
                    ...
                    />
                    <Appbar.Content title={data.alias} />
                </Appbar.Header>
                <ScrollView
